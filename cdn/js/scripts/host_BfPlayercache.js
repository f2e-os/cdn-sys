<!--
 ...
 -->
  ...
  define("host/BfPlayercache/conf", function(require, exports, module){
    var global = require("global");
    var lang   = require('fan_langBf');
    lang.updateHostXLConfCache();

    exports.page = "pages/host/BfPlayercache/conf.html";


 // 用户网段规则
  Handlebars.registerHelper('host_BfPlayercache_Conf_Reging', function(value) {
    return (value && (['服务','放行'])[value])||'-';
  });



  Handlebars.registerHelper('host_BfPlayercache_Conf_ppcList', function(value) {
  ...
  // 用户网段
  Handlebars.registerHelper('host_BfPlayercache_Conf_clientpoolOptioning', function(value){
  ...
  exports.execute = function(toModule, param){
    ...
    //清除设备组缓存/PPC 
    lang.global.updateCache("/ApiBfPlayercacheBaseConfig/listHostGroup");
      ...
      if(el.length>0)
      {
        ...
        $.dialog('是否要自动计算？', {title:'自动计算'}).yes(function(){
          $.ajax({
            type:'POST',
            dataType:'json',
            url:global.base+'/ApiBfPlayerCacheBaseConfig/autoCalc',
            data:'ids='+id.join(','),
 ...
 -->
    ...
    Fuiing.on('click','.btn-set',function(){
      ...
      $.ajax({
        type:'POST',
        dataType:'json',
        url:global.base+'/ApiBfPlayerCacheBaseConfig/setPpcConfig',
        data:getConfDataing(),
        success:function(result){
          if(result.code==1)
          {
...
<!--
 ...
 -->
     ...
     lang.setSending(global.base,toModule,{url:global.base+'/ApiBfPlayerCacheBaseConfig/getHostByType/type?type=2,5',rows:'info'},'device_ids',{getData:function(){return getConfDataing();}});
...
define('fan_langBf',function(require, exports, module){
    ...
    $(selector.body).on('click',selector.btn,function(){
      ...
      var dialog = $.dialog(template, {title:'请选择'+item.name} ).yes(function(){
        ...
        $.ajax({
          type:'POST',
          dataType:'json',
          url:global.base+'/ApiBfPlayerCacheBaseConfig/sendPpcConfig',
          data:data,
          success:function(result){
            if(result.code==1)
            {
              $.alert('下发成功').yes(function(){
                      ...
                      toModule('host/BfPlayercache/conf');
    ...
    $(selector.body).on('click',selector.btn,function(){
      $.dialog('是否要回滚？', {title:'回滚'}).yes(function(){
        $.ajax({
          type:'POST',
          dataType:'json',
          url:global.base+'/ApiBfPlayerCacheBaseConfig/rollbackPpcConfig',
          success:function(result){
            if(result.code==1)
            {                                                         
