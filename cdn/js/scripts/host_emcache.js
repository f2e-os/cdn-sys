<!--
 ...
 -->
  ...
  define("host/emcache/conf", function(require, exports, module){
    var global = require("global");
    var lang   = require('fan_langem');
    lang.updateHostEMConfCache();

    exports.page = "pages/host/emcache/conf.html";



 // 用户网段规则
  Handlebars.registerHelper('host_emcache_Conf_Reging', function(value) {
    return (value && (['服务','放行'])[value])||'-';
  });


Handlebars.registerHelper('host_emcache_Conf_ppcList', function(value) {
    return lang.renderTemplate('#ppclist-template',value);
  });

  // 限速配置
  Handlebars.registerHelper('host_emcache_Conf_limitListing', function(value) {
    return lang.renderTemplate('#limitlist-template',value);
  });


  // 用户网段
  Handlebars.registerHelper('host_emcache_Conf_clientpoolOptioning', function(value) {
  ...
  exports.execute = function(toModule, param){
    ...
    //清除设备组缓存/PPC 
    lang.global.updateCache("/ApiEMCacheBaseConfig/listHostGroup");
      ...
      if(el.length>0)
      {
        ...
        $.dialog('是否要自动计算？', {title:'自动计算'}).yes(function(){
          $.ajax({
            type:'POST',
            dataType:'json',
            url:global.base+'/ApiEMCacheBaseConfig/autoCalc',
            data:'ids='+id.join(','),
 ...
 -->
    ...
    Fuiing.on('click','.btn-set',function(){
      ...
      $.ajax({
        type:'POST',
        dataType:'json',
        url:global.base+'/ApiEMCacheBaseConfig/setPpcConfig',
        //url:config.conf.set+'/ApiEMCacheBaseConfig/setPpcConfig',
        data:getConfDataing(),
        success:function(result){
          if(result.code==1)
          {
 ...
 -->
        $('.auto-submit').on('click', function(e){
            ...
            $.ajax({
                type:'POST',
                dataType:'json',
                url:global.base + '/ApiEMCacheBaseConfig/setFileLimit',
                ...
                success:function(data){
                    ...
                    /*  //注释了
                        ...
                        var dialog = $.dialog(form,{title:"Hi.确定更改限制大小嘛？"}).yes(function(){                   
                            $.ajax({
                                url:require('global').base + '/ApiEMCacheBaseConfig/setFileLimit',
                                data:form.serialize(),
                                type:"POST",
                                dataType:"json",
...
<!--
 ...
 -->
     ...
     lang.setSending(global.base,toModule,{url:global.base+'/ApiEMCacheBaseConfig/getHostByType/type?type=2,5',rows:'info'},'device_ids',{getData:function(){return getConfDataing();}});
...
define('fan_langem',function(require, exports, module){
  var global = require('global');


  //此处有用
  exports.updateHostEMConfCache=function(){
    ...
    $(selector.body).on('click',selector.btn,function(){
      ...
      var dialog = $.dialog(template, {title:'请选择'+item.name} ).yes(function(){
        ...
        $.ajax({
          type:'POST',
          dataType:'json',
          url:global.base+'/ApiEMCacheBaseConfig/sendPpcConfig',
          data:data,
          success:function(result){
            if(result.code==1)
            {
    ...
    $(selector.body).on('click',selector.btn,function(){
      $.dialog('是否要回滚？', {title:'回滚'}).yes(function(){
        $.ajax({
          type:'POST',
          dataType:'json',
          url:global.base+'/ApiEMCacheBaseConfig/rollbackPpcConfig',
          success:function(result){
            if(result.code==1)
            {                                                         
