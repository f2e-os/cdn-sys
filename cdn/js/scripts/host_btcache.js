<!--
 ...
 -->
  ...
  define("host/btcache/conf", function(require, exports, module){
    var global = require("global");
    var lang   = require('fan_lang2');
    lang.updateHostBtConfCache();

    exports.page = "pages/host/btcache/conf.html";



 // 用户网段规则
  Handlebars.registerHelper('host_httpcache_Conf_Reging', function(value) {
    return (value && (['服务','放行'])[value])||'-';
  });


  //LBE设备列表
  Handlebars.registerHelper('host_LBEcache_Conf_LBEList', function(value) {
...
Handlebars.registerHelper('host_httpcache_Conf_ppcList', function(value) {
  ...
  // 限速配置
  Handlebars.registerHelper('host_httpcache_Conf_limitListing', function(value) {
    return lang.renderTemplate('#limitlist-template',value);
  });


  // 用户网段
  Handlebars.registerHelper('host_httpcache_Conf_clientpoolOptioning', function(value) {
  ...
  exports.execute = function(toModule, param){
    ...
    //清除设备组缓存/PPC 
    lang.global.updateCache("/ApiBtCacheBaseConfig/listHostGroup");
      ...
      if(el.length>0)
      {
        ...
        $.dialog('是否要自动计算？', {title:'自动计算'}).yes(function(){
          $.ajax({
            ...
            dataType:'json',
           // url:config.autoCalc,
            url:global.base+'/ApiBtCacheBaseConfig/autoCalc',
            data:'ids='+id.join(','),
...
<!--LBE-->
    ...
    //遍历每一个名字为的复选框，其中选中的执行函数  
     
      $('input[name="lbe"]:checked').each(function(){    
        //将选中的值添加到数组array中      
           array.push($(this).val());   
        });

      confData.lbe=array.join('|');
      array=[];
 ...
 -->
    ...
    Fuiing.on('click','.btn-set',function(){
      ...
      $.ajax({
        type:'POST',
        dataType:'json',
        url:global.base+'/ApiBtCacheBaseConfig/setPpcConfig',
        //url:config.conf.set+'/ApiBtCacheBaseConfig/setPpcConfig',
        data:getConfDataing(),
        success:function(result){
          if(result.code==1)
          {
 ...
 -->
        $('.auto-submit').on('click', function(e){
            ...
            $.ajax({
                type:'POST',
                dataType:'json',
                url:global.base + '/ApiBtCacheBaseConfig/setFileLimit',
...
<!--
 ...
 -->
     ...
     lang.setSending(global.base,toModule,{url:global.base+'/ApiBtCacheBaseConfig/getHostByType/type?type=2,5',rows:'info'},'device_ids',{getData:function(){return getConfDataing();}});
...
define('fan_lang2',function(require, exports, module){
  var global = require('global');


  //此处有用
  exports.updateHostBtConfCache=function(){
    ...
    $(selector.body).on('click',selector.btn,function(){
      ...
      var dialog = $.dialog(template, {title:'请选择'+item.name} ).yes(function(){
        ...
        $.ajax({
          type:'POST',
          dataType:'json',
          url:global.base+'/ApiBtCacheBaseConfig/sendPpcConfig',
          //url:global.base+'/ApiPpLiveCacheBaseConfig/sendPpLiveConfig',
          data:data,
          success:function(result){
            if(result.code==1)
            {
              //$.alert('下发成功').yes(function(){callback();toModule(global.base);});// 进行调试BuG
          ...
          error:function(){alert('下发失败- 我再host_btcache.js页面');}
        });
        return false;
      });
    ...
    $(selector.body).on('click',selector.btn,function(){
      $.dialog('是否要回滚？', {title:'回滚'}).yes(function(){
        $.ajax({
          type:'POST',
          dataType:'json',
          url:global.base+'/ApiBtCacheBaseConfig/rollbackPpcConfig',
          success:function(result){
            if(result.code==1)
            {                                                         
