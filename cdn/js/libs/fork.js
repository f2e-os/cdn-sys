define("fork", function(require, exports, module) {
    ...
    var render = function(data){
        ...
        if(data.id ){ dt.setAttribute("data-id", data.id); } 
        if(data.init){ dt.className = "dt-empty"; }
        dl.appendChild(dt);

        for (var i = 0; $.isArray(data.fork) && i < data.fork.length; i++) {
            var dd = create("dd"),
                ii = create("i");
    ...
    var position = function(dl){
        ...
        if( dl.parentNode.tagName.toUpperCase() !== "DD" ){
            ...
            dl.parentNode.style.width = items * 100 + 'px';
        }
    ...
    var Fork = function( holder ){
        ...
        addClass(this.holder, "fork-holder");
        // 设置元数据, 并渲染标签结构
